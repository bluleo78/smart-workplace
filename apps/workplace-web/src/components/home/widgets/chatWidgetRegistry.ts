...
// ---------------------------------------------------------------------------
// 챗 위젯 레지스트리 — AI 비서 응답(show_* 도구)이 지시한 위젯을 챗 도크에서 렌더한다.
// 대시보드 레지스트리(registry.ts)와 분리: 이쪽은 chat done 이벤트의 widgets[] 를 그린다.
...
export function getChatWidget(type: string): WidgetComponent | undefined {
  return chatWidgetRegistry[type as WidgetType];
}
