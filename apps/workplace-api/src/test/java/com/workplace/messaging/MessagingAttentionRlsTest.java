 ...
 *
 * <p>비-@Transactional 클래스: onChannelMessageSync 가 자체 트랜잭션을 열어 실제 커밋·GUC 주입 경로를 태운다.
 * (#444 교훈) 테스트 하니스 세션 GUC=1 마스킹 주의 — 존재하지 않는 대체 tenant ID(999999)로 GUC 를 전환해 격리 검증.
 * (app_tenant 롤은 tenant 행 DELETE 불가이므로 INSERT 없이 non-existent ID 사용.)
 */
@SpringBootTest
@ActiveProfiles("test")
class MessagingAttentionRlsTest {
   ...
   *
   * <p>절차:
   * 1) tenant 1 GUC 로 채널·멤버·메시지 시드 (커밋)
   * 2) aiClient 스텁 — 멤버 userId 를 relevant 로 반환
   * 3) TenantContext=1 설정 → onChannelMessageSync 호출 → 자체 tx 커밋
   * 4) GUC=1 → conversation_attention 1행 확인
  ...
  void tenant1_분류는_tenant1_행만_생성_타_tenant_격리() {
    ...
    long memberId = new TransactionTemplate(txManager).execute(status -> {
      setGuc(1L);
      // 채널 소유자 생성
    ...
    new TransactionTemplate(txManager).executeWithoutResult(status -> {
      ...
      int cnt = dsl.fetchCount(
          dsl.selectFrom(CONVERSATION_ATTENTION)
              .where(CONVERSATION_ATTENTION.CHANNEL_ID.eq(testChannelId)));
      assertThat(cnt)
          .as("tenant 1 GUC → conversation_attention 1행 기대")
          .isEqualTo(1);
    });
    ...
    new TransactionTemplate(txManager).executeWithoutResult(status -> {
      ...
      int cnt = dsl.fetchCount(
          dsl.selectFrom(CONVERSATION_ATTENTION)
              .where(CONVERSATION_ATTENTION.CHANNEL_ID.eq(testChannelId)));
      assertThat(cnt)
          .as("다른 tenant GUC → conversation_attention 0행 기대(RLS 격리)")
          .isZero();
    });
  }
  ...
  /**
   * UUID suffix 유니크 유저 INSERT 후 ID 반환.
   * 호출 시 GUC 가 이미 설정된 트랜잭션 안에서 실행되어야 RLS WITH CHECK 통과.
   */
