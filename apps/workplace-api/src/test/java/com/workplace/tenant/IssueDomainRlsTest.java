...
/**
 * V48 issue 도메인 RLS 격리 증명: 한 테넌트의 project 는 다른 테넌트 GUC 컨텍스트에서 비가시. 전체를 롤백되는 단일 트랜잭션으로 수행 → 공유 DB
 * 무오염 (app_tenant 는 tenant 행 DELETE 불가, V46; 롤백으로 미커밋 tenant/project/user 행 모두 사라짐).
 */
class IssueDomainRlsTest extends IntegrationTestBase {
  ...
  /**
   * 트랜잭션-로컬 GUC 를 직접 전환하며, tenant#2 의 project 가 tenant#1 컨텍스트에서 안 보임을 증명. 트랜잭션 내에 신규 테넌트와 user,
   * project 를 삽입하고 GUC 전환으로 RLS 격리를 확인한 뒤 롤백.
   */
  @Test
    ...
    new TransactionTemplate(txManager)
        .execute(
            status -> {
              ...
              // tid2 컨텍스트에서는 가시
              assertThat(dsl.fetchCount(dsl.selectFrom(PROJECT).where(PROJECT.ID.eq(pid))))
                  .isEqualTo(1);
  ...
  /**
   * issue 본체도 크로스테넌트 격리됨을 증명 — 가장 빈번한 조회 경로의 회귀 가드. project 테스트와 동일한 단일 트랜잭션+롤백 패턴 사용: 공유 DB 무오염.
    ...
    new TransactionTemplate(txManager)
        .execute(
            status -> {
              ...
              // tid2 컨텍스트에서는 가시
              assertThat(dsl.fetchCount(dsl.selectFrom(ISSUE).where(ISSUE.ID.eq(iid))))
                  .isEqualTo(1);
