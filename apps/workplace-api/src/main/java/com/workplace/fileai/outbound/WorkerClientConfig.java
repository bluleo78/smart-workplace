...
public class WorkerClientConfig {
   ...
   *
   * <p>HTTP/1.1 고정: 워커는 uvicorn(h11) 으로 HTTP/2 cleartext(h2c) 업그레이드를 지원하지 않는다. JDK
   * HttpClient 기본값(HTTP/2)으로 평문 호출하면 h2c 업그레이드 헤더를 보내 워커가 400("Invalid HTTP request")으로
   * 거부한다. 다른 내부 클라이언트(WorkerEmbedClient·DriveOverviewStreamClient)와 동일하게 HTTP_1_1 로 고정한다.
   */
  @Bean
