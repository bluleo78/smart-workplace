 ...
 * 캘린더 자동 동기화 스케줄러.
 *
 * <p>10분마다(fixedDelay=600s) 모든 활성 테넌트의 메일 계정을 순회하며 캘린더 동기화를 수행한다. 스케줄러는 요청 밖 스레드에서 실행되므로 HTTP 필터가
 ...
 *
 * <p>설계 의도: {@code sync()} 는 Graph API 네트워크 I/O를 포함하므로 감싸는 트랜잭션을 두지 않는다. 감싸면 커넥션이 HTTP 왕복 내내
 * idle-in-transaction 상태가 되어 커넥션 풀을 소모한다. {@code findActiveForSync()}
 * 자체가 @Transactional(readOnly=true)이고, sync 내부도 짧은 트랜잭션 분리를 사용하므로 GUC 주입은 각 트랜잭션 시작 시
 * TenantAwareTransactionManager가 담당한다.
 *
 * <p>메일 자동 동기화({@link com.workplace.mail.service.MailAutoSyncScheduler})와 동일한 패턴이며, 캐던스만 10분으로 다르다.
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class CalendarAutoSyncScheduler {
  ...
  /**
   * 스케줄 진입점 — 이전 사이클 완료 후 10분 뒤(fixedDelay) 재실행. 기동 후 60초 뒤 첫 실행(initialDelay)으로 애플리케이션 초기화를 충분히
   * 기다린다.
   */
  ...
  /**
   * 전 활성 테넌트를 순회하며 활성 계정의 캘린더를 동기화한다. 테스트에서 직접 호출하는 진입점.
   *
   * <p>테넌트 루프:
   *
   * <ol>
   ...
   *   <li>계정별 {@link CalendarSyncService#sync} 호출 — try/catch로 격리해 한 계정 실패가 다른 계정/테넌트를 막지 않도록
  ...
  public void syncAllTenants() {
    ...
    log.debug("캘린더 자동 동기화 시작 — 활성 테넌트 {}개", tenantIds.size());
    ...
    for (Long tenantId : tenantIds) {
      try {
        ...
        log.debug("테넌트 {} 활성 계정 {}개 캘린더 동기화", tenantId, accounts.size());
        ...
        for (ActiveAccount a : accounts) {
          try {
            ...
            log.warn("테넌트 {} 계정 {} 캘린더 자동 동기화 실패 — 건너뜀", tenantId, a.accountId(), e);
          }
        }
      } finally {
