 ...
 *
 * <p>비용 깔때기:
 * ① 도메인 게이트(DM 제외) → ② 이름 프리필터(무료, 멤버 이름 토큰 없으면 AI 미호출) →
 * ③ watermark 게이트(버스트 코얼레싱: 마지막 분류 이후 새 메시지 없으면 AI 0) →
 * ④ 배치 조회 → ⑤ AI(haiku) 1회 호출 → ⑥ 결과로 fan-out.
 *
...
public class MessagingAttentionService {
  ...
  /**
   * 비동기 진입점 — @Async 스레드에서 새 트랜잭션을 열어 tenant GUC 를 주입한다.
   * (작성자 스레드 컨텍스트에 의존하지 않음 — fail-safe RLS).
   */
  ...
  /**
   * 테스트·동기 호출용 본체. 비용 깔때기 ①~⑥.
   * 트리거 메시지 id 는 받지 않는다 — watermark 비교는 본체에서 maxMessageId(channelId)로 재조회한다.
  ...
  public void onChannelMessageSync(long channelId) {
    ...
    if (members.isEmpty()) { wmRepo.advance(channelId, maxId); return; }
    ...
    if (batch.isEmpty()) { wmRepo.advance(channelId, maxId); return; }
    ...
    boolean anyName = batch.stream().anyMatch(msg ->
        msg.body() != null
    ...
    var req = new MessagingClassifyRequest(
        batch.stream()
