...
public class WikiPageRepository {
   ...
   *
   * <p><b>{@code version}/{@code updated_at}/{@code updated_by} 는 절대 건드리지 않는다.</b>
   * {@link #saveIfVersion} 처럼 version 을 올리면, 스트림 도중/직후 진행 중이던 에디터가 들고 있던
   * 낙관적 동시성 버전이 서버 기준으로 낡아져 뒤이은 자동저장 PUT 이 매번 {@code affected == 0} →
   * 충돌 처리되는 회귀가 생긴다(AI 생성 = 사람 편집 아님).
   */
