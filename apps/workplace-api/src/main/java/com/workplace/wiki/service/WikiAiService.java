...
public class WikiAiService {
  ...
  /**
   * attribution 은 부가 신호일 뿐이라 실패해도 스트림 완료/에러 이벤트를 막지 않는다(§3 실패 격리) —
   * 예외를 삼키고 경고 로그만 남긴다. {@code pageService.recordAiUsage} 는 외부 빈(WikiPageService) 호출이라
   * {@code @Transactional} 프록시가 정상 적용되고(자기호출 아님), 실행 스레드는 wikiAiStreamExecutor 의
   * TenantContextTaskDecorator 가 이미 tenant 를 복원해둔 상태라 RLS GUC 도 정상 주입된다.
   */
