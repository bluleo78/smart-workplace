...
/**
 * 이슈 목록·요약 응답 DTO. projectKey 는 컨텍스트(프로젝트)에서 주입한다. labels/attachmentCount/assignees 는 호출자가 채우지 않으면
 * 빈 리스트 / 0.
 */
public record IssueResponse(
  ...
  /** projectKey + 내부 row → 응답 변환. labels/assignees 빈 리스트, attachmentCount 0 — Phase 1·2 호출자 호환. */
  ...
  /** projectKey + 내부 row + 라벨 → 응답 변환. attachmentCount 0, assignees 빈 리스트 — Phase 3a 호출자 호환. */
  ...
  /** 라벨 + 첨부 카운트까지 채운 버전 — Phase 3b 호출자 호환. assignees 는 빈 리스트. */
  ...
  /** 라벨 + 첨부 카운트 + 담당자까지 모두 채운 풀버전 — Phase 3c 검색/상세에서 사용. */
